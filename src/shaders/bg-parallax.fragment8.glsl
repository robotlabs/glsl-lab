...
// FUNZIONE MODIFICATA: rimosso liquidTime e semplificata
float terrainHeight(float x, int li) {
  ...
  // Usa solo warp statico basato sul layer
  baseUv = warp2(baseUv, float(li) * 12.34);
  
  // FIXED: Velocità diversa per ogni layer per effetto caotico!
  float layerSpeed = 0.4 + float(li) * 0.15; // Ogni layer ha velocità diversa
  float layerOffset = float(li) * 3.7; // Offset più distanziato per più caos
  
  float speedFluid = 1.0;
  float safeTime = mod(uTime * layerSpeed + layerOffset, 6.318) ;
  float liquidPhase = sin(safeTime) * 0.2* speedFluid;;
...
void main(){
  ...
  // Tempo per gli sky objects (mantenuto)
  float t = mod(uTime * 0.2, 1000.0);
  // TEMPO PRINCIPALE per il movimento orizzontale delle curve
  float horizontalTime = uTime / 5.0; // Rimuovi la moltiplicazione per mantenere la velocità naturale
  ...
  for (int i=0; i<MAX_LAYERS; i++) {
    ...
    // CALCOLO PARALLASSE: ogni layer si muove alla sua velocità
    float world_x = st.x + horizontalTime * uSpeed[i] + uOffset[i];
    float h = terrainHeight(world_x, i); // Rimosso liquidTime

    // Noise statico per variazione (opzionale, puoi ridurlo o rimuoverlo)
    float bigNoise = noise(st * 10.0 * uSmoothness[i]) * 0.02; // Ridotto
    float medNoise = noise(st * 15.0 * uSmoothness[i]) * 0.01; // Ridotto
    float smallNoise = noise(st * 25.0 * uSmoothness[i]) * 0.005; // Ridotto
    ...
    if (st.y < noisyHeight) {
      ...
      // Distorsione ridotta per riflessi più stabili
    ...
    // Border BIANCO PURO sopra al layer (semplificato)
    float distFromSurface = st.y - noisyHeight;
    
    // Variazione statica per il thickness del border
    float variation1 = noise(vec2(world_x * 8.0, 0.0)) * 0.5 + 0.5;
    float variableThickness = mix(0.001, 0.008, variation1);
    
    // Border sempre visibile ma con thickness variabile
