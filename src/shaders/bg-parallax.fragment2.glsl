...
float terrainHeight(float x, int li) {
  ...
  // --- tua rotazione "jiggle", ma SOLO per il dettaglio ---
  float a = 0.4 + 0.1 * uTime / (40.0 + float(li));//sin(uTime * 0.02 + float(li));
  vec2 uvDetail = rot2(a) * uv * 1.0;   // <— ruoto SOLO il dettaglio
  // ---------------------------------------------------------

  #ifdef NOISE_WARPED
    uv       = warp(uv);
    uvDetail = warp(uvDetail);    // opzionale: se vuoi lo stesso warp anche sul dettaglio
  #endif

  // grande struttura (silhouette stabile = scroll costante)
  float nLarge  = fbm_base(uv * 0.4);     // <-- invariato
  // dettaglio fine (balla leggero)
  float nDetail = fbm_base(uvDetail * 0.8);

  // blend — 0.3 = quanto dettaglio ci metti (come avevi)
  float n = mix(nLarge, nDetail, 0.4);
